...
var APWS = require('apocket-api');

var Common = require('../lib/common');
var Constants = Common.Constants;
var Utils = Common.Utils;
var Client = require('../lib');
var ExpressApp = APWS.ExpressApp;
var Storage = APWS.Storage;
    ...
    async.series([
        ...
        function(next) {
          async.each(_.range(1, n), function(i, cb) {
            ...
            clients[i].joinWallet(secret, 'pocketeer ' + i, {
              coin: coin
            }, cb);
          }, next);
        },
...
describe('client API', function() {
    ...
    it('should fetch notifications at intervals', function(done) {
      helpers.createAndJoinWallet(clients, 2, 2, function() {
        ...
        clients[0]._fetchLatestNotifications(5, function() {
          _.pluck(notifications, 'type').should.deep.equal(['NewPocketeer', 'WalletComplete']);
          clock.tick(2000);
          notifications = [];
          clients[0]._fetchLatestNotifications(5, function() {
  ...
  describe('Wallet Creation', function() {
    ...
    it('should encrypt pocketeer name in wallet creation', function(done) {
      var spy = sinon.spy(clients[0], '_doPostRequest');
      clients[0].seedFromRandomWithMnemonic();
      clients[0].createWallet('mywallet', 'pepe', 1, 1, {}, function(err, secret) {
        should.not.exist(err);
        ...
        url.should.contain('/pocketeers');
        body.should.not.contain('pepe');
        clients[0].getStatus({}, function(err, status) {
          should.not.exist(err);
          status.wallet.pocketeers[0].name.should.equal('pepe');
          done();
        })
      });
    });
    it('should be able to access wallet name in non-encrypted wallet (legacy)', function(done) {
      ...
      clients[0]._doPostRequest('/v2/wallets/', args, function(err, wallet) {
        ...
        var hash = Utils.getPocketeerHash(args.name, args.xPubKey, args.requestPubKey);
        args.pocketeerSignature = Utils.signMessage(hash, wpk);
        clients[0]._doPostRequest('/v2/wallets/123/pocketeers', args, function(err, wallet) {
          should.not.exist(err);
          clients[0].openWallet(function(err) {
            ...
            clients[0].getStatus({}, function(err, status) {
              ...
              wallet.pocketeers[0].name.should.equal('pepe');
              should.not.exist(wallet.pocketeers[0].encryptedName);
              done();
            });
          });
        });
    ...
    it('should be able to complete wallet in pocketeer that joined later', function(done) {
      ...
      clients[0].createWallet('mywallet', 'creator', 2, 2, {
        ...
        clients[1].joinWallet(secret, 'guest', {}, function(err, wallet) {
          ...
          clients[0].openWallet(function(err, walletStatus) {
            ...
            _.difference(_.pluck(walletStatus.pocketeers, 'name'), ['creator', 'guest']).length.should.equal(0);
            if (++checks == 2) done();
          });
        });
      });
    ...
    it('should return wallet on successful join', function(done) {
      clients[0].createWallet('mywallet', 'creator', 2, 2, {
        ...
        clients[1].joinWallet(secret, 'guest', {}, function(err, wallet) {
          should.not.exist(err);
          wallet.name.should.equal('mywallet');
          wallet.pocketeers[0].name.should.equal('creator');
          wallet.pocketeers[1].name.should.equal('guest');
          done();
        });
      });
    });
    ...
    it('should not allow to join a full wallet ', function(done) {
      helpers.createAndJoinWallet(clients, 2, 2, function(w) {
        ...
        clients[4].joinWallet(w.secret, 'pocketeer', {}, function(err, result) {
    ...
    it('should fail with an invalid secret', function(done) {
      // Invalid
      clients[0].joinWallet('dummy', 'pocketeer', {}, function(err, result) {
        err.message.should.contain('Invalid secret');
        // Right length, invalid char for base 58
        clients[0].joinWallet('DsZbqNQQ9LrTKU8EknR7gFKyCQMPg2UUHNPZ1BzM5EbJwjRZaUNBfNtdWLluuFc0f7f7sTCkh7T', 'pocketeer', {}, function(err, result) {
          err.message.should.contain('Invalid secret');
          done();
        });
      });
    ...
    it('should fail with an unknown secret', function(done) {
      ...
      clients[0].joinWallet(oldSecret, 'pocketeer', {}, function(err, result) {
    ...
    it('should detect wallets with bad signatures', function(done) {
      ...
      helpers.createAndJoinWallet(clients, 2, 3, function() {
        helpers.tamperResponse([clients[0], clients[1]], 'get', '/v1/wallets/', {}, function(status) {
          status.wallet.pocketeers[0].xPubKey = status.wallet.pocketeers[1].xPubKey;
        }, function() {
    ...
    it('should detect wallets with missing signatures', function(done) {
      ...
      helpers.createAndJoinWallet(clients, 2, 3, function() {
        helpers.tamperResponse([clients[0], clients[1]], 'get', '/v1/wallets/', {}, function(status) {
          delete status.wallet.pocketeers[1].xPubKey;
        }, function() {
          openWalletStub.restore();
          clients[1].openWallet(function(err, x) {
            err.should.be.an.instanceOf(Errors.SERVER_COMPROMISED);
    ...
    it('should detect wallets missing callers pubkey', function(done) {
      ...
      helpers.createAndJoinWallet(clients, 2, 3, function() {
        helpers.tamperResponse([clients[0], clients[1]], 'get', '/v1/wallets/', {}, function(status) {
          ...
          status.wallet.pocketeers[1].xPubKey = (new Bitcore.HDPrivateKey()).publicKey;
          // Add a correct signature
          status.wallet.pocketeers[1].xPubKeySignature = Utils.signMessage(
            status.wallet.pocketeers[1].xPubKey.toString(),
    ...
    it('should set walletPrivKey from APWS', function(done) {
      clients[0].createWallet('mywallet', 'creator', 1, 1, {
        network: 'testnet'
      }, function(err) {
  ...
  describe('Address Creation', function() {
    ...
    it('should be able to create address in all pocketeers in a 2-3 wallet', function(done) {
      this.timeout(5000);
      helpers.createAndJoinWallet(clients, 2, 3, function() {
        clients[0].createAddress(function(err, x) {
          should.not.exist(err);
  ...
  describe('Notifications', function() {
    ...
    it('should receive notifications', function(done) {
      clients[0].getNotifications({}, function(err, notifications) {
        ...
        _.pluck(notifications, 'type').should.deep.equal(['NewPocketeer', 'WalletComplete', 'NewAddress']);
    ...
    it('should not receive notifications for self generated events unless specified', function(done) {
      clients[0].getNotifications({}, function(err, notifications) {
        ...
        clients[0].getNotifications({
          ...
          _.pluck(notifications, 'type').should.deep.equal(['NewPocketeer', 'NewPocketeer', 'WalletComplete', 'NewAddress', 'NewAddress']);
          done();
        });
      });
    });
        ...
        clients[0].getTxProposals({}, function(err, txps) {
          ...
          clients[0].publishTxProposal({
            ...
            clients[0].getTxProposals({}, function(err, txps) {
              ...
              // Should be visible for other pocketeers as well
      ...
      clients[0].createTxProposal(opts, function(err, txp) {
        ...
        clients[0].publishTxProposal({
          ...
          clients[0].getTxProposals({}, function(err, txps) {
            should.not.exist(err);
            txps.length.should.equal(1);
            // Try to republish from pocketeer 1
      ...
      helpers.createAndPublishTxProposal(clients[0], opts, function(err, x) {
        ...
        clients[1].rejectTxProposal(x, 'rejection comment', function(err, tx1) {
          ...
          clients[2].getTxProposals({}, function(err, txs) {
            ...
            txs[0].actions[0].pocketeerName.should.equal('pocketeer 1');
    ...
    describe('Shared wallet', function() {
      ...
      it('Should Create and Verify a Tx from PayPro', function(done) {
        clients[1].getTxProposals({}, function(err, txps) {
          ...
          tx.message.should.equal('Payment request for AriseBank invoice CibEJJtG1t9H77KmM61E2t for merchant testaPocket');
      ...
      it('Should send the "payment message" when last pocketeer sign', function(done) {
        clients[0].getTxProposals({}, function(err, txps) {
          should.not.exist(err);
          clients[0].signTxProposal(txps[0], function(err, xx, paypro) {
            ...
            clients[1].signTxProposal(xx, function(err, yy, paypro) {
              ...
              clients[1].broadcastTxProposal(yy, function(err, zz, memo) {
                ...
                memo.should.equal('Transaction received by AriseBank. Invoice will be marked as paid if the transaction is confirmed.');
                zz.message.should.equal('Payment request for AriseBank invoice CibEJJtG1t9H77KmM61E2t for merchant testaPocket');
                done();
              });
            });
          });
  ...
  describe('Transactions Signatures and Rejection', function() {
        ...
        clients[0].createAddress(function(err, x0) {
          ...
          helpers.createAndPublishTxProposal(clients[0], opts, function(err, txp) {
            ...
            clients[0].getStatus({}, function(err, st) {
              ...
              var w = st.wallet;
              w.pocketeers.length.should.equal(3);
    ...
    it.skip('Send, reject actions in 2-3 wallet must have correct pocketeerNames', function(done) {
      helpers.createAndJoinWallet(clients, 2, 3, function(w) {
        clients[0].createAddress(function(err, x0) {
          should.not.exist(err);
          blockchainExplorerMock.setUtxo(x0, 10, 2);
          ...
          helpers.createAndPublishTxProposal(clients[0], opts, function(err, txp) {
            ...
            clients[0].signTxProposal(txp, function(err, txp) {
              ...
              clients[0].signTxProposal(txp, function(err) {
                should.exist(err);
                err.should.be.an.instanceOf(Errors.POCKETEER_VOTED);
    ...
    it('should edit a note for an arbitrary txid', function(done) {
      clients[0].editTxNote({
        ...
        clients[0].getTxNote({
          ...
          note.body.should.equal('note body');
          note.editedBy.should.equal(clients[0].credentials.pocketeerId);
          note.editedByName.should.equal(clients[0].credentials.pocketeerName);
  ...
  describe('Mobility, backup & restore', function() {
    describe('Export & Import', function() {
      ...
      describe('Compliant derivation', function() {
        ...
        it('should export & import with mnemonics + APWS', function(done) {
          var c = clients[0].credentials;
          var walletId = c.walletId;
          var walletName = c.walletName;
          var pocketeerName = c.pocketeerName;
          var key = c.xPrivKey;
          ...
          importedClient.importFromMnemonic(exported, {
            ...
            c2.pocketeerName.should.equal(pocketeerName);
            done();
          });
        });

        it('should export & import with xprivkey + APWS', function(done) {
          var c = clients[0].credentials;
          var walletId = c.walletId;
          var walletName = c.walletName;
          var pocketeerName = c.pocketeerName;
      ...
      describe('Non-compliant derivation', function() {
        ...
        it('should check APWS once if specific derivation is not problematic', function(done) {
    ...
    describe('Mnemonic related tests', function() {
      ...
      it('should import with mnemonics livenet', function(done) {
        ...
        client.createWallet('mywallet', 'creator', 1, 1, {
          ...
          importedClient.importFromMnemonic(exported, {}, function(err) {
            ...
            c2.pocketeerName.should.equal(c.pocketeerName);
            done();
          });
        });
      });
      // Generated with https://dcpos.github.io/bip39/
      it('should fail to import from words if not at APWS', function(done) {
      ...
      it('should fail to import from words if not at APWS, with passphrase', function(done) {
        var exported = 'bounce tonight little spy earn void nominee ankle walk ten type update';
        importedClient = helpers.newClient(app);
        importedClient.importFromMnemonic(exported, {
          network: 'testnet',
    ...
    describe('Recovery', function() {
      it('should be able to gain access to a 1-1 wallet with just the xPriv', function(done) {
        helpers.createAndJoinWallet(clients, 1, 1, function() {
          ...
          var pocketeerName = clients[0].credentials.pocketeerName;
          ...
          clients[0].createAddress(function(err, addr) {
            ...
            recoveryClient.openWallet(function(err) {
              ...
              recoveryClient.credentials.pocketeerName.should.equal(pocketeerName);
            ...
            expressApp.start({
              ...
              recoveryClient.getStatus({}, function(err, status) {
                ...
                recoveryClient.recreateWallet(function(err) {
                  ...
                  // Do not send wallet name and pocketeer names in clear text
                  var url = spy.getCall(0).args[0];
                  var body = JSON.stringify(spy.getCall(0).args[1]);
                  url.should.contain('/wallets');
                  body.should.not.contain('mywallet');
                  ...
                  body.should.not.contain('pocketeer 1');

                  recoveryClient.getStatus({}, function(err, status) {
                    should.not.exist(err);
                    status.wallet.name.should.equal('mywallet');
                    _.difference(_.pluck(status.wallet.pocketeers, 'name'), ['creator', 'pocketeer 1']).length.should.equal(0);
  ...
  describe('Legacy aPocket Import', function() {
    it('Should get wallets from profile', function(done) {
      var t = ImportData.pocketeers[0];
      var c = helpers.newClient(app);
      var ids = c.getWalletIdsFromOldaPocket(t.username, t.password, t.ls['profile::4872dd8b2ceaa54f922e8e6ba6a8eaa77b488721']);
    ...
    it('Should import a 1-1 wallet', function(done) {
      var t = ImportData.pocketeers[0];
      var c = helpers.newClient(app);
      c.createWalletFromOldaPocket(t.username, t.password, t.ls['wallet::e2c2d72024979ded'], function(err) {
        should.not.exist(err);
        ...
        c.createAddress(function(err, x0) {
          ...
          c.getStatus({}, function(err, status) {
            ...
            status.wallet.pocketeers.length.should.equal(1);
            status.wallet.pocketeers[0].name.should.equal('123');
            done();
          });
        });
      });
    ...
    it('Should to import the same wallet twice with different clients', function(done) {
      var t = ImportData.pocketeers[0];
      var c = helpers.newClient(app);
      c.createWalletFromOldaPocket(t.username, t.password, t.ls['wallet::4d32f0737a05f072'], function(err) {
        should.not.exist(err);
        c.getStatus({}, function(err, status) {
          should.not.exist(err);
          status.wallet.status.should.equal('complete');
          ...
          c2.createWalletFromOldaPocket(t.username, t.password, t.ls['wallet::4d32f0737a05f072'], function(err) {
            should.not.exist(err);
    ...
    it('Should not fail when importing the same wallet twice, same pocketeer', function(done) {
      var t = ImportData.pocketeers[0];
      var c = helpers.newClient(app);
      c.createWalletFromOldaPocket(t.username, t.password, t.ls['wallet::4d32f0737a05f072'], function(err) {
        should.not.exist(err);
    ...
    it('Should import and complete 2-2 wallet from 2 pocketeers, and create addresses', function(done) {
      var t = ImportData.pocketeers[0];
      var c = helpers.newClient(app);
      c.createWalletFromOldaPocket(t.username, t.password, t.ls['wallet::4d32f0737a05f072'], function(err) {
        should.not.exist(err);
        c.getStatus({}, function(err, status) {
          ...
          var t2 = ImportData.pocketeers[1];
          var c2 = helpers.newClient(app);
          c2.createWalletFromOldaPocket(t2.username, t2.password, t2.ls['wallet::4d32f0737a05f072'], function(err) {
            should.not.exist(err);
    ...
    it('Should import and complete 2-3 wallet from 2 pocketeers, and create addresses', function(done) {
      var w = 'wallet::7065a73486c8cb5d';
      var key = 'fS4HhoRd25KJY4VpNpO1jg==';
      var t = ImportData.pocketeers[0];
      var c = helpers.newClient(app);
      c.createWalletFromOldaPocket(t.username, t.password, t.ls[w], function(err) {
        should.not.exist(err);
        c.getStatus({}, function(err, status) {
          should.not.exist(err);
          status.wallet.status.should.equal('complete');
          ...
          var t2 = ImportData.pocketeers[1];
          var c2 = helpers.newClient(app);
          c2.createWalletFromOldaPocket(t2.username, t2.password, t2.ls[w], function(err) {
            should.not.exist(err);
            ...
            c2.getStatus({}, function(err, status) {
              ...
              var t3 = ImportData.pocketeers[2];
              var c3 = helpers.newClient(app);
              c3.createWalletFromOldaPocket(t3.username, t3.password, t3.ls[w], function(err) {
                should.not.exist(err);
    ...
    it('Should import a 2-3 wallet from 2 pocketeers, and recreate it, and then on the recreated other pocketeers should be able to access', function(done) {
      var w = 'wallet::7065a73486c8cb5d';
      var key = 'fS4HhoRd25KJY4VpNpO1jg==';
      var t = ImportData.pocketeers[0];
      var c = helpers.newClient(app);
      c.createWalletFromOldaPocket(t.username, t.password, t.ls[w], function(err) {
        ...
        c2.createWalletFromOldaPocket(t2.username, t2.password, t2.ls[w], function(err) {
          ...
          recoveryClient.recreateWallet(function(err) {
            ...
            recoveryClient.getStatus({}, function(err, status) {
              should.not.exist(err);
              _.pluck(status.wallet.pocketeers, 'name').sort().should.deep.equal(['123', '234', '345']);
              var t2 = ImportData.pocketeers[1];
              var c2p = helpers.newClient(newApp);
              c2p.createWalletFromOldaPocket(t2.username, t2.password, t2.ls[w], function(err) {
                should.not.exist(err);
  ...
  describe('Private key encryption', function() {
    ...
    it('should export & import encrypted', function(done) {
      ...
      var pocketeerName = c1.credentials.pocketeerName;
      ...
      importedClient.openWallet(function(err) {
        ...
        importedClient.credentials.pocketeerName.should.equal(pocketeerName);
  ...
  describe('#addAccess', function() {
    describe('1-1 wallets', function() {
      ...
      it('should add access with pocketeer name', function(done) {
        var spy = sinon.spy(clients[0], '_doPutRequest');
        clients[0].addAccess({
          name: 'pepe',
        }, function(err, x, key) {
          ...
          clients[0].getStatus({}, function(err, status) {
            should.not.exist(err);
            var keys = status.wallet.pocketeers[0].requestPubKeys;
