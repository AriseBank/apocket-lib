 ...
 *
 * @param {Function} credentials
 * @param {Array} pocketeers
 * @returns {Boolean} true or false
 */
Verifier.checkPocketeers = function(credentials, pocketeers) {
  ...
  if (pocketeers.length != credentials.n) {
  ...
  var uniq = [];
  var error;
  _.each(pocketeers, function(pocketeer) {
    if (error) return;

    if (uniq[pocketeers.xPubKey]++) {
      log.error('Repeated public keys in server response');
      error = true;
    }

    // Not signed pub keys
    if (!(pocketeer.encryptedName || pocketeer.name) || !pocketeer.xPubKey || !pocketeer.requestPubKey || !pocketeer.signature) {
      log.error('Missing pocketeer fields in server response');
      error = true;
    } else {
      var hash = Utils.getPocketeerHash(pocketeer.encryptedName || pocketeer.name, pocketeer.xPubKey, pocketeer.requestPubKey);
      if (!Utils.verifyMessage(hash, pocketeer.signature, walletPubKey)) {
  ...
  if (!_.includes(_.pluck(pocketeers, 'xPubKey'), credentials.xPubKey)) {
...
Verifier.checkTxProposalSignature = function(credentials, txp) {
  ...
  var creatorKeys = _.find(credentials.publicKeyRing, function(item) {
    if (Utils.xPubToPocketeerId(txp.coin || 'btc', item.xPubKey) === txp.creatorId) return true;
  });
