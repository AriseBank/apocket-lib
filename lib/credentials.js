...
Credentials.prototype._expand = function() {
  ...
  this.pocketeerId = Utils.xPubToPocketeerId(this.coin, this.xPubKey);
  this.publicKeyRing = [{
...
Credentials.prototype.addWalletInfo = function(walletId, walletName, m, n, pocketeerName) {
  ...
  if (pocketeerName)
    this.pocketeerName = pocketeerName;
...
Credentials.fromOldaPocketWallet = function(w) {
  function walletPrivKeyFromOldaPocketWallet(w) {
    // IN APWS, the master Pub Keys are not sent to the server,
    // so it is safe to use them as seed for wallet's shared secret.
    var seed = w.publicKeyRing.pocketeersExtPubKeys.sort().join('');
  ...
  credentials.addWalletPrivateKey(walletPrivKeyFromOldaPocketWallet(w));
  credentials.addWalletInfo(w.opts.id, w.opts.name, w.opts.requiredPocketeers, w.opts.totalPocketeers)

  var pkr = _.map(w.publicKeyRing.pocketeersExtPubKeys, function(xPubStr) {
    ...
    var pocketeerName = w.publicKeyRing.nicknameFor[pubKey];
    if (isMe) {
      credentials.pocketeerName = pocketeerName;
    }

    return {
      xPubKey: xPubStr,
      requestPubKey: requestDerivation.publicKey.toString(),
      pocketeerName: pocketeerName,
    };
  });
