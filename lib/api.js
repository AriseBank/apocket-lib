...
API.prototype._processTxps = function(txps) {
  ...
  _.each([].concat(txps), function(txp) {
    ...
    _.each(txp.actions, function(action) {
      action.pocketeerName = API._decryptMessage(action.pocketeerName, encryptingKey);
      action.comment = API._decryptMessage(action.comment, encryptingKey);
      // TODO get pocketeerName from Credentials -> pocketeerId to pocketeerName
      // action.pocketeerName = null;
    });
...
API.prototype.openWallet = function(cb) {
  ...
  self._doGetRequest('/v2/wallets/?includeExtendedInfo=1', function(err, ret) {
    ...
    if (!self.credentials.hasWalletInfo()) {
      var me = _.find(wallet.pocketeers, {
        id: self.credentials.pocketeerId
      });
    ...
    if (self.credentials.walletPrivKey) {
      if (!Verifier.checkPocketeers(self.credentials, wallet.pocketeers)) {
        return cb(new Errors.SERVER_COMPROMISED);
      }
    } else {
      // this should only happen in AIR-GAPPED flows
      log.warn('Could not verify pocketeers key (missing wallet Private Key)');
    }

    self.credentials.addPublicKeyRing(API._extractPublicKeyRing(wallet.pocketeers));
...
API.prototype._doRequest = function(method, url, args, useSession, cb) {
  ...
  if (self.credentials) {
    headers['x-identity'] = self.credentials.pocketeerId;
 ...
 * @param {String} pocketeerName
 * @param {Object} Optional args
 * @param {String} opts.customData
 * @param {String} opts.coin
 * @param {Callback} cb
 */
API.prototype._doJoinWallet = function(walletId, walletPrivKey, xPubKey, requestPubKey, pocketeerName, opts, cb) {
  $.shouldBeFunction(cb);
  var self = this;
  ...
  var encPocketeerName = Utils.encryptMessage(pocketeerName, this.credentials.sharedEncryptingKey);

  var args = {
    walletId: walletId,
    coin: opts.coin,
    name: encPocketeerName,
    xPubKey: xPubKey,
  ...
  args.pocketeerSignature = Utils.signMessage(hash, walletPrivKey);

  var url = '/v2/wallets/' + walletId + '/pocketeers';
...
API._extractPublicKeyRing = function(pocketeers) {
  return _.map(pocketeers, function(pocketeer) {
    var pkr = _.pick(pocketeer, ['xPubKey', 'requestPubKey']);
    pkr.pocketeerName = pocketeer.name;
    return pkr;
  });
};
 ...
 * @return {undefined}
 */
API.prototype.createWallet = function(walletName, pocketeerName, m, n, opts, cb) {
  var self = this;
  ...
  self._doPostRequest('/v2/wallets/', args, function(err, res) {
    ...
    c.addWalletInfo(walletId, walletName, m, n, pocketeerName);
    var secret = API._buildSecret(c.walletId, c.walletPrivKey, c.coin, c.network);

    self._doJoinWallet(walletId, walletPrivKey, c.xPubKey, c.requestPubKey, pocketeerName, {
        coin: coin
      },
      function(err, wallet) {
        if (err) return cb(err);
 ...
 */
API.prototype.joinWallet = function(secret, pocketeerName, opts, cb) {
  var self = this;
  ...
  self._doJoinWallet(secretData.walletId, secretData.walletPrivKey, self.credentials.xPubKey, self.credentials.requestPubKey, pocketeerName, {
    coin: coin,
    dryRun: !!opts.dryRun,
  }, function(err, wallet) {
    if (err) return cb(err);
    if (!opts.dryRun) {
      self.credentials.addWalletInfo(wallet.id, wallet.name, wallet.m, wallet.n, pocketeerName);
    }
  ...
  this.getStatus({
    ...
    self._doPostRequest('/v2/wallets/', args, function(err, body) {
      ...
      async.each(self.credentials.publicKeyRing, function(item, next) {
        var name = item.pocketeerName || ('pocketeer ' + i++);
        self._doJoinWallet(walletId, walletPrivKey, item.xPubKey, item.requestPubKey, name, {
          coin: c.coin,
          supportBIP44AndP2PKH: supportBIP44AndP2PKH,
        }, function(err) {
          //Ignore error is pocketeer already in wallet
          if (err && err instanceof Errors.POCKETEER_IN_WALLET) return next();
          return next(err);
        });
      }, cb);
    });
...
API.prototype._processWallet = function(wallet) {
  ...
  _.each(wallet.pocketeers, function(pocketeer) {
    var name = Utils.decryptMessage(pocketeer.name, encryptingKey);
    if (name != pocketeer.name) {
      pocketeer.encryptedName = pocketeer.name;
    }
    pocketeer.name = name;
    _.each(pocketeer.requestPubKeys, function(access) {
...
API.prototype._processStatus = function(status) {
  ...
  function processCustomData(data) {
    var pocketeers = data.wallet.pocketeers;
    if (!pocketeers) return;

    var me = _.find(pocketeers, {
      'id': self.credentials.pocketeerId
    });
 ...
 * @param {String} opts.includeOwn[=false] (optional) - Do not ignore notifications generated by the current pocketeer
...
API.prototype.getNotifications = function(opts, cb) {
  ...
  self._doGetRequestWithLogin(url, function(err, result) {
    ...
    var notifications = _.filter(result, function(notification) {
      return opts.includeOwn || (notification.creatorId != self.credentials.pocketeerId);
    });
 ...
 * When finished, the scanning process will send a notification 'ScanFinished' to all pocketeers.
 *
 * @param {Object} opts
...
API.prototype.startScan = function(opts, cb) {
  ...
  var args = {
    includePocketeerBranches: opts.includePocketeerBranches,
  };
...
/**
 * Adds access to the current pocketeer
 * @param {Object} opts
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var pocketeerId = this.credentials.pocketeerId;

  var encPocketeerName = opts.name ? Utils.encryptMessage(opts.name, this.credentials.sharedEncryptingKey) : null;
  ...
  this._doPutRequest('/v1/pocketeers/' + pocketeerId + '/', opts, function(err, res) {
    if (err) return cb(err);
 ...
 * @param {String} [opts.provider] - A provider of exchange rates (default 'AriseBank').
...
API.prototype._oldaPocketDecrypt = function(username, password, blob) {
  var SEP1 = '@#$';
  var SEP2 = '%^#@';
...
API.prototype.getWalletIdsFromOldaPocket = function(username, password, blob) {
  var p = this._oldaPocketDecrypt(username, password, blob);
  if (!p) return null;
 ...
 * @return {undefined}
 */
API.prototype.createWalletFromOldaPocket = function(username, password, blob, cb) {
  var self = this;
  var w = this._oldaPocketDecrypt(username, password, blob);
  if (!w) return cb(new Error('Could not decrypt'));

  if (w.publicKeyRing.pocketeersExtPubKeys.length != w.opts.totalPocketeers)
    return cb(new Error('Wallet is incomplete, cannot be imported'));

  this.credentials = Credentials.fromOldaPocketWallet(w);
